...
<head>
    ...
    <title>Syntax Certificates - Add Students</title>
    ...
    <meta name="description" content="Syntax E-Certificate - Add Students.">
...
<body class="bg-[#f0f3ff]" style="font-family: 'Inter';">
    ...
    <main class="pt-28 md:pl-64 p-6">
        ...
        <div class="bg-white rounded-lg shadow-[0_5px_50px_-20px_rgba(0,0,0,0.3)] p-6 mb-8">
            <form method="POST" action={{ route('students.add.post' )}} enctype="multipart/form-data">
                @csrf
                <div class="space-y-6">
                    <div>
                        <label for="nis" class="block text-sm font-medium text-gray-700 mb-1">Student NIS <span class="text-red-400">*</span></label>
                        <input type="text" name="nis" id="nis" placeholder="Enter student nis (eg: 222310114)" class="w-full px-4 py-2 rounded-lg border border-gray-200 focus:outline-none focus:border-[#35448f]">
                        @error('nis')
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" placeholder="Enter student name (eg: Yazid Yusuf)" class="w-full px-4 py-2 rounded-lg border border-gray-200 focus:outline-none focus:border-[#35448f]">
                        @error('name')
                            <p class="text-red-500 text-sm">* {{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="major" id="major" placeholder="Enter student major (eg: REKAYASA PERANGKAT LUNAK)" class="w-full px-4 py-2 rounded-lg border border-gray-200 focus:outline-none focus:border-[#35448f]">
                        @error('major')
                            <p class="text-red-500 text-sm">* {{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="batch" id="batch" placeholder="Enter student batch (eg: 15)" class="w-full px-4 py-2 rounded-lg border border-gray-200 focus:outline-none focus:border-[#35448f]">
                        @error('batch')
                            <p class="text-red-500 text-sm">* {{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="flex justify-end gap-4 mt-8">
                    ...
                    <button type="submit" class="px-6 py-2 bg-[#35448f] text-white rounded-lg hover:bg-[#435197]">Add Student</button>
                </div>
            </form>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-[0_5px_50px_-20px_rgba(0,0,0,0.3)] p-6 mt-8">
            <form method="POST" action={{ route('students.add.post.multiple' )}} enctype="multipart/form-data">
                @csrf
                <div class="space-y-6">
                    <div>
                        <label for="data" class="block text-sm font-medium text-gray-700 mb-1">Students Data (CSV) <span class="text-red-400">*</span></label>
                        ...
                        <p class="text-gray-500 text-sm">Perlu template data CSV? <a href="/files/template-students.csv" class="text-[#35448f] hover:text-[#435197]">Klik Disini</a> untuk mengunduh template</p>
                    </div>
                </div>

                <div class="flex justify-end gap-4 mt-8">
                    <a href={{ route('students') }} class="px-6 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">Cancel</a>
                    <button type="submit" class="px-6 py-2 bg-[#35448f] text-white rounded-lg hover:bg-[#435197]">Add Students</button>
                </div>
            </form>
        </div>
    </main>
