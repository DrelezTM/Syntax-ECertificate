...
<head>
    ...
    <title>Syntax Community - Students</title>
    ...
    <meta name="description" content="Syntax E-Certificate - Student List.">
...
<body class="bg-[#f0f3ff]" style="font-family: 'Inter';">
    ...
    <main class="pt-28 md:pl-64 p-6">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
            <form method="GET" action="{{ route('students') }}" class="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
            ...
            <a href="/students/add" class="bg-[#35448f] text-white hover:bg-[#435197] px-4 py-2 rounded-lg flex items-center gap-2 transition-colors">
        ...
        <div class="bg-white rounded-lg shadow-[0_5px_50px_-20px_rgba(0,0,0,0.3)] overflow-hidden">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div>
                                        <div class="text-sm font-medium text-gray-900">{{ $student->major }}</div>
                                        <div class="text-sm text-gray-500">{{ $student->academic_year }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-500">{{ $student->batch }}</td>
                            <td class="px-6 py-4 text-sm">
                                <div class="flex gap-5">
                                    <a href="{{ route('students.edit') }}?id={{ $student->id }}" class="text-yellow-500 hover:text-yellow-600"><i class="fa-solid fa-pen-to-square"></i></a>
                                    <button class="text-red-600 hover:text-red-800" onclick="deleteItem({{ $student->id }})"><i class="fa-solid fa-trash"></i></button>
                                </div>
                            </td>
                        </tr>

                        <form id="delete-form-{{ $student->id }}" action="{{ route('students.delete') }}?id={{ $student->id }}" method="POST" style="display: none;">
                            @csrf
                            @method('DELETE')
                        </form>
                        @empty
            ...
            <div class="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
                <div class="flex-1 flex justify-between sm:hidden">
                    ...
                    @else
                        <a href="{{ $students->previousPageUrl() }}" class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50">Previous</a>
                    @endif
            
                    @if ($students->hasMorePages())
                        <a href="{{ $students->nextPageUrl() }}" class="ml-3 relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50">Next</a>
                    @else
                    ...
                    <div>
                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                            ...
                            @for ($i = $start; $i <= $end; $i++)
                                ...
                                @else
                                    <a href="{{ $students->url($i) }}" class="relative inline-flex items-center px-4 py-2 border border-gray-300 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50">{{ $i }}</a>
                                @endif
                            @endfor
